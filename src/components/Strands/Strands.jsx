...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const initialWidth = Math.max(ctn.offsetWidth || 300, 10);
    const initialHeight = Math.max(ctn.offsetHeight || 300, 10);
    ...
    function resize() {
      if (!ctn) return;
      const width = Math.max(ctn.offsetWidth || 300, 10);
      const height = Math.max(ctn.offsetHeight || 300, 10);
