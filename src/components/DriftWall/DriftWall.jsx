...
const DEFAULT_ITEMS = Array.from({ length: 10 }, (_, i) => ({
  id: `${i + 1}`,
  image: `https://picsum.photos/id/${1015 + i}/600/400`,
...
const DriftWall = ({
  ...
  const columnItems = useMemo(() => {
    ...
    items.forEach((item, i) => cols[i % safeColumns].push(item));
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span
        ...
        style={{
          '--card-bg': item.cardBg || '#ffffff',
          '--card-text': item.cardTextColor || '#04193a',
          '--brand-color': item.color || '#ffd026',
        }}
      >
        <span className="drift-wall__poster-frame">
          <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
          <span className="drift-wall__overlay" aria-hidden="true" />
        </span>

        <span className="drift-wall__bottom">
          {item.title && <span className="drift-wall__title">{item.title}</span>}
          <span className="drift-wall__footer-action">
    ...
    const commonProps = {
      ...
      onKeyDown: (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          handleTileClick(item, e);
        }
      }
    };

    if (item.href && !item.onClick && !onTileClick) {
      return (
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Drifting wall of brand tiles"
    >
